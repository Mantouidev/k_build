...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^mantoui_") ; then
       CUSTOM_BUILD=$(echo -n $1 | sed -e 's/^mantoui_//g')
    else
       CUSTOM_BUILD=
    fi
    export CUSTOM_BUILD
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/mantoui/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
